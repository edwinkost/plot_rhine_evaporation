...
# evaporation:
evaporation_file = "totalEvaporation_annuaTot_output.2003.laea.map"
evaporation      = readGDAL(evaporation_file)				 	
evaporation_dtfr = as(evaporation,"data.frame")

# 
theme_set(theme_bw())
outplot <- ggplot() +
 geom_tile(data = evaporation_dtfr, aes(x = x, y = y,  fill = band1))
