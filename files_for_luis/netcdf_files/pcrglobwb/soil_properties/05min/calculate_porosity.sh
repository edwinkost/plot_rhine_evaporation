...
cp /projects/0/dfguu/data/hydroworld/PCRGLOBWB20/input5min/landSurface/soil/v03Jul2013/soilProperties5ArcMin.nc .

# landmask
cp /projects/0/dfguu/data/hydroworld/PCRGLOBWB20/input5min/routing/lddsound_05min.map .
pcrcalc landmask.map = "defined(lddsound_05min.map)"

# for the first layer of PCR-GLOBWB, soil depth/thickness: <= 30 cm
# - saturated volumetric moisture content (m3.m-3)
cdo select,name=satVolWC1 soilProperties5ArcMin.nc satVolWC1.nc
...
gdal_translate -of NETCDF satVolWC1.map satVolWC1_5ArcMin.nc

# for the second layer of PCR-GLOBWB, soil depth/thickness: <= 120 cm
# - saturated volumetric moisture content (m3.m-3)
cdo select,name=satVolWC2 soilProperties5ArcMin.nc satVolWC2.nc
...
gdal_translate -of NETCDF satVolWC2.map satVolWC2_5ArcMin.nc

# - soil depth/thickness for the first layer 
cdo select,name=firstStorDepth soilProperties5ArcMin.nc firstStorDepth.nc
...
cdo select,name=secondStorDepth soilProperties5ArcMin.nc secondStorDepth.nc
...
cdo setname,satVolWC satVolWC.nc satVolWC_5ArcMin.nc
rm satVolWC.nc
